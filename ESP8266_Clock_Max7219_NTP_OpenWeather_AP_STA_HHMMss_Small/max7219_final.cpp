...
extern uint8_t scr[32]; // Declarado em GlobalDefines.cpp (32 bytes)
...
void scrollLeft() {
  // CORREÇÃO: Ajustar o loop para o tamanho correto do array
  // scr tem tamanho 32 (NUM_MAX * 8), então o loop deve ir até 31
  for (int i = 0; i < NUM_MAX * 8 - 1; i++) {
    scr[i] = scr[i + 1];
  }
  // O último elemento recebe 0
  scr[NUM_MAX * 8 - 1] = 0;
}
...
void initMAX7219() {
  ...
  // CORREÇÃO CRÍTICA: shutdown=0 desliga, shutdown=1 liga
  sendCmdAll(CMD_DISPLAYTEST, 0);   // Teste OFF
  sendCmdAll(CMD_SCANLIMIT, 7);     // 8 linhas
  sendCmdAll(CMD_DECODEMODE, 0);    // No decode
  sendCmdAll(CMD_INTENSITY, 8);     // Brilho médio (0-15)
  sendCmdAll(CMD_SHUTDOWN, 1);      // LIGADO (1 = normal operation)
